...
contract OpeningCeremonyQuest is AccessControl {
  ...
  function setConditions(bytes32 _setEnclaveCondition, bytes32 _setCompletedCondition)
    external
  ...
  function setCurrentStepWithLock(
    address templar,
    bytes32 step,
    uint256 lockDurationSeconds,
    bytes32 stepWhenUnlocked
  ) external onlyRole(CAN_CHANGE_STATE) {
    QuestData storage questData = dataOf[templar];
  ...
  function setEnclave(address templar, bytes32 enclave) external onlyRole(CAN_CHANGE_STATE) {
    require(
      keccak256(abi.encode(getCurrentStep(templar))) == setEnclaveCondition && dataOf[templar].enclave == "",
      "Cannot set enclave at current quest step"
    );
  ...
  function setCompleted(address templar) external onlyRole(CAN_CHANGE_STATE) {
    require(
      keccak256(abi.encode(getCurrentStep(templar))) == setCompletedCondition,
      "Cannot set quest as completed at current step"
    );
